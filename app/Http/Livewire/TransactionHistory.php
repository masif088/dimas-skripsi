...
class TransactionHistory extends Component
{
    ...
    public function mount()
    {
        ...
        $this->transactionList
            = \App\Models\Transaction::whereIn('status_order_id', [2, 3])
            ->whereDate('created_at', $this->date)->get();
        $total = 0;
        $amount = 0;
        foreach (
            \App\Models\Transaction::where('status_order_id', 2)
                ->whereDate('created_at', $this->date)->get() as $tl
        ) {
            foreach ($tl->transactionDetails as $td) {
                ...
                if (isset($this->productMethod[$td->product->product_company_id][$td->transaction->payment_method_id])) {
                    $this->productMethod[$td->product->product_company_id][$td->transaction->payment_method_id] += $td->amount
                        * $td->price;
                } else {
                    $this->productMethod[$td->product->product_company_id][$td->transaction->payment_method_id]
                        = $td->amount * $td->price;
                }
